...
// 标记类型为 IceProp
export const icePropKey = '__icePropKey'
// 获取对象值的联合类型
export type Value<T> = T[keyof T]

// 组件 Prop 类型
export type IceProp<Type, Required, Default> = {
  readonly type: PropType<Type>, // 获得 Type 的构造器类型
  readonly required: [Required] extends [true]? true : false
  [icePropKey]: true
} & IfNever<Default, unknown, { readonly default: Default }>

export type IfIceProp<T, Y, N> = T extends { [icePropKey]: true } ? Y : N
 ...
 */
export type IcePropInput<
  Type, 
  Value,
  Required extends boolean,
  Default extends IcePropMergeType<Type, Value>
> = {
  type?: Type,
  values?: readonly Value[] // 获取 values 数组元素的联合类型
  required?: Required
  default?: IcePropInputDefault<Required, Default> // 处理 default 的类型
}

/**
 * 处理 IcePropInput 的默认值类型
 * 如果 required === true，则 Default 的类型为 never，即没有默认值
 */
export type IcePropInputDefault<
  Required extends boolean,  
  Default 
...
export type IcePropMergeType<Type, Value> = 
 ...
 * buildProp 方法的返回值类型
 */
export type IcePropOutput<Type, Value, Required, Default> = IceProp<
  IcePropMergeType<Type, Value>, // Type
  Required,  // Required
  Default  // Default
>
...
/**
 * 将 buildProps 中的每一项 Input 转换为 IcePropOutput
 */
export type IcePropConvert<Input> = Input extends IcePropInput<
  infer Type,
  infer Value,
  infer Required,
  any
>
  ? IcePropOutput<Type, Value, Required, Input['default']/* 使用处理后的类型 */>
  : never